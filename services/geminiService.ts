...
const getContextInstruction = (context?: WordLevel | string, part: 'definitions' | 'story' | 'questions' = 'definitions'): string => {
    ...
    const subjectMap: Record<string, string> = {
        [WordLevel.Biologie]: `het schoolvak biologie`,
        [WordLevel.MensEnMaatschappij]: `de schoolvakken mens en maatschappij (aardrijkskunde, geschiedenis, maatschappijleer)`,
        [WordLevel.Economie]: `de schoolvakken economie, boekhouden en ondernemerschap`,
        [WordLevel.Wiskunde]: `het schoolvak wiskunde (rekenen, algebra, meetkunde, etc.)`,
        [WordLevel.Natuurkunde]: `de schoolvakken natuurkunde (fysica) en techniek`,
        [WordLevel.PeriodiekSysteem]: `chemie en het periodiek systeem der elementen`,
...
export const generateQuizQuestions = async (
  ...
  if (settings.context === WordLevel.Engels) {
    prompt = `Genereer een quiz met multiple-choice vragen in het **Engels** om Engelse "Parts of Speech" te toetsen. De doelgroep zijn Nederlandse sprekers die Engels leren.

**Context:**
${contextString}

**Instructies voor de vragen:**
1.  **Taal:** De vragen ('vraag') en de antwoordopties ('opties') moeten in het **Engels** zijn.
2.  **Variatie:** Creëer verschillende soorten vragen.
3.  **Schema:** Volg het opgegeven JSON-schema nauwkeurig.

Genereer een vraag voor elk van de volgende woorden: ${words.join(', ')}.`;
  } else {
    prompt = `Genereer een quiz met multiple-choice vragen in het Nederlands voor NT2-leerlingen. Je krijgt een lijst van woorden en hun Frayer Model data. Maak voor **elk woord** in de lijst precies één unieke en uitdagende vraag.
